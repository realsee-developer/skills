...
当 `REALSEE_REGION` 为 `global` 时改用 `global`。只有全部文件都通过 manifest 校验后，下载器才会再次检查目标路径，并通过一次原子重命名发布完整目录。之后运行 Argus 仍会把所选图片上传到对应的 Realsee Gateway，必须另行取得用户同意。详见[官方示例全景图](references/examples.zh-CN.md)。
